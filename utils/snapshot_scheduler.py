...
async def _do_snapshot_all():
    ...
    count = len(panels)
    if count > 0:
        logger.info(f"✅ Snapshots taken for {count} panel(s)")
    else:
        logger.debug("ℹ️ No panels to snapshot")

    # ── Auto-purge snapshots older than 7 days to prevent DB bloat ──
    try:
        from datetime import timedelta
        cutoff = (datetime.utcnow() - timedelta(days=7)).isoformat()
        if is_mongo():
            db = get_db()
            await db.member_snapshots.delete_many({"taken_at": {"$lt": cutoff}})
        else:
            async with aiosqlite.connect(get_sqlite_path()) as conn:
                await conn.execute(
                    "DELETE FROM member_snapshots WHERE taken_at < ?", (cutoff,)
                )
                await conn.commit()
    except Exception as e:
