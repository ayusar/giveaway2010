...
async def form_clone_token(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"🔐 <b>You must be an Admin of the channel</b> — this is verified automatically.\n\n"
        f"Tap <b>Skip</b> if you don't want a join gate.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⏭ Skip (no join gate)", callback_data="skip_channel")]
        ])
    )


@router.callback_query(F.data == "retry_channel_link")
async def retry_channel_link(callback: CallbackQuery, state: FSMContext):
    """Prompt user to re-enter channel after failed admin check."""
    await callback.answer()
    await callback.message.edit_text(
        "🔄 <b>Please enter your channel username again:</b>\n\n"
        "Example: <code>@mychannel</code>\n\n"
        "Make sure you are an <b>Admin</b> of the channel first.",
        parse_mode="HTML",
    )
    await state.set_state(CloneBotForm.channel_link)
...
async def clone_cancel(callback: CallbackQuery, state: FSMContext):
    """Cancel the clone bot setup flow."""
    await callback.answer()
    await state.clear()
    await callback.message.edit_text(
        "❌ <b>Setup cancelled.</b>\n\n"
        "Use /clonebot whenever you're ready to try again.",
        parse_mode="HTML",
    )
...
async def form_channel_link(message: Message, state: FSMContext, bot: Bot):
    link = message.text.strip()
    if not link.startswith("@") and not link.startswith("https://t.me/"):
        await message.answer(
            "❌ Please enter a valid channel like <code>@mychannel</code> "
            "or <code>https://t.me/mychannel</code>",
            parse_mode="HTML",
        )
        return

    # Normalise t.me links to @username
    if link.startswith("https://t.me/"):
        link = "@" + link.split("https://t.me/")[1].lstrip("/").split("/")[0]

    # ── Security: verify user is actually admin of this channel ──
    verifying_msg = await message.answer(
        "🔍 <b>Verifying your admin status in the channel…</b>",
        parse_mode="HTML",
    )
    from utils.channel_admin_check import verify_channel_admin
    ok, err = await verify_channel_admin(bot, message.from_user.id, link)
    try:
    ...
    if not ok:
        await message.answer(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔄 Try Again", callback_data="retry_channel_link")],
